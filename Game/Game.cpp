...
void Game::Load()
{
	...
	// Create the font to display the health.
	m_healthFont = new Font("Cosmic Sans MS", 12, FW_BOLD);
	m_scoreBoardFont = new Font( "Cosmic Sans MS", 12, FW_BOLD );
...
void Game::Close()
{
	...
	// Destroy the font used for the health.
	SAFE_DELETE( m_healthFont );
...
void Game::Update( float elapsed )
{
	...
	// Allow the player to respawn by pressing the L key.
	if( g_engine->GetInput()->GetKeyPress( DIK_L ) == true )
	{
		m_playerManager->GetLocalPlayer()->SetEnabled( false );
		m_playerManager->SpawnLocalPlayer();
	}

	m_localHealth = m_playerManager->GetLocalPlayer()->GetHealth();
	sprintf( m_healthText, "%d", m_localHealth);
	...
	if( g_engine->GetInput()->GetKeyPress( DIK_TAB, true ) == true )
	{
		...
		sprintf_s( m_scoreBoardNames, "PLAYER\n" );
		sprintf_s( m_scoreBoardFrags, "KILLS\n" );
		sprintf_s( m_scoreBoardDeaths, "DEATHS\n" );
		...
		while( player != NULL )
		{
			...
			sprintf_s( m_scoreBoardFrags, "%s%d", m_scoreBoardFrags, player->GetFrags() );
			strcat( m_scoreBoardFrags, "\n" );

			sprintf_s( m_scoreBoardDeaths, "%s%d", m_scoreBoardDeaths, player->GetDeaths() );
...
void Game::Render()
{
	...
	//render health
	m_healthFont->Render( m_healthText, 100, 0 );
	...
	if( g_engine->GetInput()->GetKeyPress( DIK_TAB, true ) == true )
	{
				...
				m_scoreBoardFont->Render( m_scoreBoardDeaths, 300, 100, 0xFFFF7700 );
	}
